...
public class ThreePotSystemTest {
    ...
    private static Stream<Arguments> constellations() {
        ...
        final int LOWER_THAN_KONTO1_MIN = KONTO1_MIN_VALUE - 1000;
        final int LOWER_THAN_KONTO1_OPT = KONTO1_OPT_VALUE - 1000;
        final int LOWER_THAN_KONTO2_MIN = KONTO2_MIN_VALUE - 1000;
        final int LOWER_THAN_KONTO2_OPT = KONTO2_OPT_VALUE - 1000;
    ...
    public void testConstellations(String title, int pot1, int pot2,
        ...
        System.out.println();
        System.out.printf("================= %s (Toleranz +/-%d) =================%n", title, TOLERANCE);
        System.out.printf("Check Konto 1: %d = %d\n", state.getPot1(), expectedMinPot1);
        assertAlmostEquals(state.getPot1(), expectedMinPot1, String.format("Konto 1 falsch: %d >= expected %d", state.getPot1(), expectedMinPot1));
        System.out.printf("Check Konto 2: %d = %d\n", state.getPot2(),expectedMinPot2);
        assertAlmostEquals(state.getPot2(), expectedMinPot2, String.format("Konto 2 falsch: %d >= expected %d", state.getPot2(), expectedMinPot2));
        ...
        assertAlmostEquals(pot1 + pot2 + pot3, state.getPot1() + state.getPot2() + newPot3, "Kontostand falsch");
    }
