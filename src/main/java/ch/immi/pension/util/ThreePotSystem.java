...
public class ThreePotSystem {
    ...
    public void updateState(ThreePotState state) {
        if (state.getPot1() < potThreshold.getPot1Min()) {
            ...
            if (state.getPot2() < potThreshold.getPot2Min()) {
                ...
                buySharesFromPots(state, pot1DownToOptAmount, pot2DownToOptAmount);
            }
        }
    ...
    private void sellSharesForPots(ThreePotState state, int pot1Amount, int pot2Amount) {
        if (pot1Amount > 0) {
            int sharesToSell3a = (int)((pot1Amount * pot3aPercentage) / state.getPrice3a());
            int sharesToSell3b = (int)((pot1Amount * pot3bPercentage) / state.getPrice3b());
            state.moveFrom3To1(sharesToSell3a, sharesToSell3b);
        }
        if (pot2Amount > 0) {
            int sharesToSell3a = (int)((pot2Amount * pot3aPercentage) / state.getPrice3a());
            int sharesToSell3b = (int)((pot2Amount * pot3bPercentage) / state.getPrice3b());
            state.moveFrom3To2(sharesToSell3a, sharesToSell3b);
        }
    }

    private void buySharesFromPots(ThreePotState state, int pot1Amount, int pot2Amount) {
        if (pot1Amount > 0) {
            int sharesToBuy3a = (int)(pot1Amount * pot3aPercentage / state.getPrice3a());
            int sharesToBuy3b = (int)(pot1Amount * pot3bPercentage / state.getPrice3b());
            state.moveFrom1To3(sharesToBuy3a, sharesToBuy3b);
        }
        if (pot2Amount > 0) {
